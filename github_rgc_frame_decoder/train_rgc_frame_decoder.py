...
def parse_args():
    ...
    parser.add_argument("--decoder", type=str, default="attention", choices=["linear", "conv", "attention", "dense_ae", "neural_image", "freq_split"])
    ...
    parser.add_argument("--loss-mode", type=str, default="rf_ssim_wlmse_gabor", choices=["mse_ssim", "rf_ssim_wlmse_gabor", "ssim_mse", "ssim_grad", "adversarial"])
...
def compute_temporal_smoothness(model, dataset, device):
    ...
    with torch.no_grad():
        ...
        pred = torch.sigmoid(model(x.to(device))[0]).cpu()
    inter_frame_sim = []
    ...
    with torch.no_grad():
        for x, y, prev_y in tqdm(loader, desc=f"eval {mode}", leave=False):
            ...
            if mode == "normal":
                ...
                pred = torch.sigmoid(model(x[perm])[0])
                ...
                pred = torch.sigmoid(model(control_x)[0])
